...
**TaigaBot Lite** — Telegram-бот, который молча слушает один конкретный чат 24/7, собирает сообщения в локальную базу данных, а раз в сутки генерирует хулиганскую текстовую сводку дня через OpenAI API и публикует её обратно в чат.

Это учебная (облегчённая) версия боевого проекта TAIGA. Здесь нет Docker, нет PostgreSQL, нет картинок. Только чистая логика, минимум зависимостей, максимум понятности.
...
├── llm.py                  # обёртка над OpenAI API + системный промпт
│
...
4. Сохраняем результат в `summaries`.
5. Отправляем текст сводки в `ALLOWED_CHAT_ID`.
6. Удаляем из `messages` все записи старше 48 часов.
...
1. **МЕМ ДНЯ** — главный образ суток. Формат: *"Если бы сегодняшний чат был картинкой, это было бы..."* + развёрнутое описание образа (кот, житель тайги, известная личность).
