...
Используется **JS API** Telegram Login Widget (`Telegram.Login.auth()` из `telegram-widget.js`). Кнопка «ВХОД» в правом верхнем углу страницы открывает popup Telegram.

Поддерживаются два формата ответа от Telegram:

1. **Новый JWT flow** — callback возвращает `{id_token}`. Бэкенд валидирует JWT:
   - Публичные ключи загружаются с JWKS endpoint: `https://oauth.telegram.org/.well-known/jwks.json`
   - Подпись проверяется через PyJWT + PyJWKClient (RS256)
   - Claims: `iss == "https://oauth.telegram.org"`, `aud == BOT_ID`, `exp` не истёк
   - **Важно:** `PyJWKClient.get_signing_key_from_jwt()` — синхронный HTTP-запрос, вызывается через `asyncio.to_thread()`

2. **Legacy hash flow** — callback возвращает `{id, first_name, hash, ...}`. Бэкенд валидирует:
